...
const Svet = ({ data, sidebar_data }) => {
  ...
  return (
    <Layout_Single>
      <Single_Page title="свет " cards={data} />
    </Layout_Single>
  );
};
...
export const getServerSideProps = async cotext => {
  try {
    const res_sidebar =  await axios(`${process.env.VESNIK_POCETNA_URL}temi`)
    const sidebar_data =await res_sidebar.data
    const res = await axios(`${process.env.VESNIK_KATEGORIJA_URL}3?_format=json`)
